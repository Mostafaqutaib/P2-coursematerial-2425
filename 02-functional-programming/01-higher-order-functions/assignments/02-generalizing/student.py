...
def select_movies_with_actor(movies, actor_name):
    result = []
    for movie in movies:
...
def sum_movie_duration_from_period(movies, start_year, end_year):
    ...
    for movie in movies:
        if start_year <= movie.year <= end_year:
            total_duration += movie.runtime  # ⚠️ ملاحظة: اسم attribute هو runtime وليس duration
    return total_duration

def get_directors_of_actor(movies, actor_name):
    directors = []
    for movie in movies:
        if actor_name in movie.actors:
...
# اختبار الدوال
print("عدد أفلام سنة 2010:", count_movies_from_year(movies, 2010))
print("عدد أفلام كريستوفر نولان:", len(select_movies_with_actor(movies, "Christopher Nolan")))
print("مدة أفلام التسعينات:", sum_movie_duration_from_period(movies, 1990, 1999))
print("مخرجو توم كروز:", get_directors_of_actor(movies, "Tom Cruise"))

# اختبار مع توم كروز (موجود في فيلم Magnolia)
tom_cruise_movies = select_movies_with_actor(movies, "Tom Cruise")
print("أفلام توم كروز:", [movie.title for movie in tom_cruise_movies])
print("مخرجو توم كروز:", get_directors_of_actor(movies, "Tom Cruise"))